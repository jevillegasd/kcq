"""TransmonStar PCell: concentric transmon qubit with configurable
star-shaped couplers.

Central circular island with N radial coupling connectors, each
individually configurable for angle, depth, gap, and width. Design based
on work by A. Wallraff et al. Migrated from PDK_Qfoundry
(qfoundry/tech/pymacros/qfoundry/qubits/TransmonStar.py), which had no
kqcircuits dependency; only the layer numbering and port marking changed
here (pins.add_pin instead of a dedicated port_layer parameter) to match
kcq's own conventions.
"""
...
class TransmonStar(pya.PCellDeclarationHelper):
    ...
    def set_parameters(self):
        ...
        self.param("metal_n_layer", self.TypeLayer, "Metalization negative (gap) layer",
        ...
        self.param("corner_radius", self.TypeDouble,
                   "Corner rounding radius for inner cutouts [um]",
    ...
    def _create_transmon_star(self):
        ...
        # Add a pin at the end of each connector
        for i in range(self.n_couplers):
            ...
            self._add_coupler_pin(i, connector_length)
    ...
    def _make_connector_waveguide(self, angle_deg, connector_length, gap=0):
        ...
        Args:
            ...
            connector_length: Extension length beyond outer_radius + gnd_clearance (um)
    ...
    def _make_ground_pocket(self, angle_deg, connector_length=0):
        ...
        Args:
            angle_deg: Rotation angle (degrees)
            connector_length: Extension length (um)
    ...
    def _add_coupler_pin(self, index, connector_length):
        """Adds a kcq pin (kcq.geometry.pins) at the end of coupler
        `index`'s waveguide extension, replacing the raw port-layer path
        the PDK_Qfoundry reference used."""
        angle_deg = self.coupler_angles[index]
        port_r = self.outer_radius + self.ground_clearance + connector_length
