"""Shared geometry helpers for the junctions/ PCells.

Ported from PDK_Qfoundry (qfoundry/tech/pymacros/qfoundry/junctions/utils.py
+ qfoundry/tech/pymacros/qfoundry/utils.py). Two changes from the source:
- `polygon_with_vsym` (there: kqcircuits.util.symmetric_polygons) is
  reimplemented locally below -- it only ever mirrored a point list
  across the y-axis and closed the polygon.
- numpy (cos/sin/radians/linspace/sign) is replaced with stdlib math,
  matching the rest of kcq's geometry code, since numpy isn't otherwise
  a dependency here.
"""
...
def polygon_with_vsym(points):
    """Polygon with vertical symmetry: mirrors `points` across the y-axis
    (x -> -x) and appends them in reverse to close the polygon."""
    mirrored = [pya.DPoint(-p.x, p.y) for p in reversed(points)]
    return pya.DPolygon([*points, *mirrored])
...
def arc(r, start=0, stop=math.pi / 2, n=64):
    """
        r: radius
        start/stop: angle in radians
        n: number of corners in full circle
    """
    n_steps = max(round(abs(stop - start) * n / (2 * math.pi)), 1)
    step = (stop - start) / n_steps
    r_corner = r / math.cos(step / 2)
    angles = _linspace(start, stop, n_steps + 2)
    points = []
    for alpha in angles:
        points.append(pya.DPoint(r_corner * math.cos(alpha), r_corner * math.sin(alpha)))
    return points
...
def draw_junction(angle, inner_angle, junction_width_b, junction_width_t, finger_size, mirror_offset, offset_compensation, finger_overshoot, finger_overlap,
  bottom_lead_comp=0, center=pya.DPoint(0, 0), dbu=0.001) -> pya.DPolygon:
    ...
    _inner_angle = math.radians(inner_angle)
    ...
    if mirror_offset:
        ddt += offset_compensation * math.cos(_angle)
    else:
        ddb += offset_compensation * math.cos(_angle)

    def finger_points(size, width, angle, lead_comp=0):
        fo_x = (finger_overshoot) * math.cos(angle)
        fo_y = finger_overshoot * math.sin(angle)
        pl_x = (finger_overlap + lead_comp) * math.cos(angle)
        pl_y = (finger_overlap + lead_comp) * math.sin(angle)

        dx = width / 2 * math.sin(angle)
        dy = width / 2 * math.cos(angle)

        end_x = size * math.cos(angle)
        end_y = size * math.sin(angle)
...
def _patch_scratches(gap, patch_width, conn_height, patch_clearance, size, angle, rot=0, round=False):
    ...
    span_y = conn_height + patch_clearance + abs(end_y)

    scratch_ang = abs(math.pi / 4 - abs(angle))
    scratch_y1 = patch_width / 2 * math.sin(scratch_ang)
    scratch_x1 = patch_width / 2
    y0 = gap / 2 + scratch_y1

    dy = scratch_w * math.cos(scratch_ang)
    dx = -scratch_w * math.sin(scratch_ang)
...
def _round_corners_and_append(polygon, polygon_list=None, rounding_params=None, dbu=0.001):
    """Rounds `polygon`'s corners, converts it to integer coordinates, and
    appends it to polygon_list (creating one if not given). No-op (returns
    polygon_list unchanged) if polygon is empty."""
    if polygon.is_empty():
        return polygon_list
    ...
    if rounding_params is None:
        rounding_params = {"rinner": 5, "router": 10, "n": 64}
...
def add_shapes(cell, shapes, layer) -> pya.Region:
    """Merges `shapes` (a single shape or a list of them) into a Region
    and inserts it on `layer` via cell.shapes(...)."""
